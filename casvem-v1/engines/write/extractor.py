...
Rules:
  - Each fact must stand alone — include the subject explicitly (replace all pronouns)
  - Split compound facts joined by "and": e.g. "I am 24 and live in Bangalore" → TWO facts
...
async def run(
    ...
    user_id: str,
    transcript: str,
    existing_l5_id: str | None = None,
) -> list[MemoryNode]:
    """
    1. Save raw session to L5 (skipped if existing_l5_id provided — lazy promotion path).
    2. Extract facts → L4 nodes.
    Returns the list of new L4 nodes inserted.
    """
    store = get_store()

    # ── L5: save raw session (skip when re-processing an existing L5 node) ────
    if existing_l5_id is None:
        l5_embed = await embedder().embed(transcript[:2000])
        l5_node  = MemoryNode(
            layer       = 5,
            content     = transcript,
            embedding   = l5_embed,
        ...
        l5_nid   = l5_node.node_id
    else:
        l5_id  = existing_l5_id
        l5_nid = existing_l5_id
